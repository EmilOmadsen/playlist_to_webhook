...
class SpotifyPlaylistGenerator:
    ...
    def send_playlist_to_webhook(self, name, description, tracks, user_info=None):
        """Send playlist data to webhook instead of creating on Spotify"""
        try:
            webhook_url = "https://thelabelsunday.app.n8n.cloud/webhook/1e2d8f4b-2ae6-4503-b331-319e9cc9c1b0"
            
            # Format tracks for webhook
            formatted_tracks = []
            for track in tracks:
                formatted_track = {
                    'id': track.get('id'),
            ...
            print(f"✅ Successfully sent playlist '{name}' to webhook")
            return {
                'name': name,
                'description': description,
            ...
            print(f"❌ Error sending playlist to webhook: {e}")
            return None
...
def generate_playlists():
    ...
    try:
        ...
        # Generate playlists and send to webhook
        created_playlists = []
        user_info = {
        ...
        for i in range(num_playlists):
            ...
            print(f"Sending playlist to webhook: {final_playlist_name} with {len(selected_tracks)} tracks")
            
            # Send playlist to webhook instead of creating on Spotify
            playlist_result = generator.send_playlist_to_webhook(
                final_playlist_name,
                final_description,
                selected_tracks,
                user_info
            ...
            if playlist_result:
                created_playlists.append(playlist_result)
                print(f"✅ Successfully sent playlist to webhook: {playlist_result.get('name')}")
            else:
                print(f"❌ Failed to send playlist to webhook: {final_playlist_name}")
        ...
        for playlist in created_playlists:
            if playlist:
                try:
                    results.append({
                        'name': playlist.get('name', 'Unknown Playlist'),
                        'tracks': playlist.get('tracks_count', 0),
                        'webhook_response': playlist.get('webhook_response', 'Unknown'),
                        'webhook_url': playlist.get('webhook_url', ''),
                        'description': playlist.get('description', ''),
                        'status': 'sent_to_webhook'
                    })
                except Exception as e:
                    print(f"Error formatting webhook playlist result: {e}")
                    continue
